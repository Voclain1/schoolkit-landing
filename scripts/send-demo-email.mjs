#!/usr/bin/env node
/**
 * Sends the "demo video" campaign to the waitlist audience as a Resend Broadcast.
 *
 *   node scripts/send-demo-email.mjs --test      # one send to TEST_RECIPIENT, no broadcast
 *   node scripts/send-demo-email.mjs --dry-run   # print the resolved recipient list, send nothing
 *   node scripts/send-demo-email.mjs --send      # create + send the live broadcast
 *
 ...
 *
 * This script never adds to or removes from the audience — the broadcast targets
 * existing membership only, so no contact.created webhooks fire.
 */
...
import {
  ...
  getDemoEmailSubject,
  getDemoEmailText,
} from "../lib/email-templates/demo-video.ts";

const __dirname = path.dirname(fileURLToPath(import.meta.url));
const ROOT = path.resolve(__dirname, "..");

const CAMPAIGN = "demo-video-v1";
...
async function main() {
  ...
  const subject = getDemoEmailSubject();
  // Merge tags only resolve on broadcasts, so the test send gets the fallbacks baked in.
  const preview = mode === "test";
  const html = getDemoEmailHtml({ preview });
  const text = getDemoEmailText({ preview });
  ...
  const created = await api("POST", "/broadcasts", {
    ...
    html,
    text,
    name: `SchoolKit demo video (${CAMPAIGN})`,
  });
